...
public interface ParqueaderoRepositorio extends CrudRepository<Parqueadero,String>{    
    ...
    @Query(value = "select r.F_FECHA_RESERVA as fecha, sum(r.Q_SUB_TOTAL) total, count(r.K_COD_RESERVA) as cantReservas from RESERVA r WHERE r.F_FECHA_RESERVA BETWEEN CURDATE() - INTERVAL 15 DAY AND CURDATE() AND r.K_COD_PARQUEADERO = ? GROUP BY r.F_FECHA_RESERVA;",nativeQuery = true)
